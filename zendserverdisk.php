 ...
 * @since		1.5
 */
class JCacheStorageZendserverdisk extends JCacheStorage
{
	/**
	 * Constructor
	 *
	...
	public function __construct($options = array())
	{
		...
		$empty = array();
		zend_disk_cache_store($this->_hash.'-index',$empty);	
	}
	/**
	...
	public function getAll()
	{
		...
		$keys = zend_disk_cache_fetch($this->_hash.'-index');
		$secret = $this->_hash;
        ...
        if (is_array($keys)) {
        	foreach ($keys as $key) {
        		...
        		if (strpos($key['name'], $secret.'-cache-'.$group.'-')===0 xor $mode != 'group') {
					zend_disk_cache_delete($key['name']);
        		}
        	}
        }
		return true;
	...
	public function lock($id,$group,$locktime)
	{
		...
		$data_lock = zend_disk_cache_store($cache_id,1,$locktime);
	...
	public function unlock($id,$group=null)
	{
		...
		$unlock = zend_disk_cache_delete($cache_id);
		return $unlock;
	}
}
